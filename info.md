...
- 每条指令可映射 **红外遥控器**（`remote.send_command`：官方 Broadlink 或 **Logitech Harmony Hub**）或 **按钮**（`button.press`）。红外设备列表按所选遥控器切换：Broadlink 读 `.storage/broadlink_remote_MACADDRESS_codes`，Harmony 读配置目录下的 `harmony_UNIQUE_ID.conf`（`Devices` 里的名称，如 Apple TV）
...
索尼没有 AirPlay / 原厂 HomeKit：官方路径是 **Bravia 集成 → HomeKit 桥接 → HA 为电视拆出配件**。把本集成的 `media_player` 加进**同一座桥**（勾选 `media_player` 域或包含该实体），家庭 App 扫 **配件** 码，不要扫主桥。
