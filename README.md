...
每条按键或输入源可以走 **红外遥控器**（`remote.send_command`：官方 Broadlink 或 Logitech Harmony Hub）或任意 **按钮实体**（`button.press`）。图形化 Config Flow / Options Flow，无需 YAML。
...
Each key or input source is either an **IR remote** (`remote.send_command`: Broadlink or Logitech Harmony Hub) or a **button entity** (`button.press`). Setup is a graphical wizard — no YAML required.
